...
{   // This is the class for the randomly appearing enemies, moving down with varying speeds, picking up walkers and lifting them up off the screen 
    class Enemy
    {            
        // Class Variables
        Texture2D m_txr;
        ...
        // Constructor
        public Enemy (Texture2D txr, Texture2D txrMap, Random RNG, int minX, int maxX)
        {                         
            m_txr = txr;
            m_txrMap = txrMap;

            // Set a random position in the X axis between the specified borders minX and maxX
            m_position = new Vector2(RNG.Next(minX, maxX), 0);
